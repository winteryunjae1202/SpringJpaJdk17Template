...
public class NoticeJoinEntity {
     ...
     * name => NoticeJoinEntitu 조인할 컬럼명 ( 즉, Notice 테이블의 user_id 컬럼과 user_info 테이블 조인)
     * UserInfoEntity, NoticeJoinEntitu 둘다 동일한 이름의 user_id 사용하고 있어
     ...
     * 외래키는 PK 값과 연결되는 구조이기에 user_info 테이블의 PK 컬림인 user_id가 조인 대상이 됨
     * 즉, user_info 테이블의 user_id 컬럼과 notice 테이블의 user_id 컬럼이 조인됨
     * <p>
     * 만약, user_info 테이블의 PK 컬럼이 아닌 user_name과 같이 다른 컬럼 사용하고 싶은 경우
     * referencedColumnName = "user_name" 사용하고, UserInfoEntitiy는 implements Serializable 구현함
     */
    @OneToOne
